angular.module('isteven-multi-select', ['ng']).directive('istevenMultiSelect', ['$sce', '$timeout', '$templateCache', function ($sce, $timeout, $templateCache) {
    ...
    return {
        ...
        scope: {
            ...
            // william adds for owner tooltip dropdown specification.
            ownerToolTip: '='
        },
        ...
        link: function ($scope, element, attrs) {
            ...
            $scope.updateFilter = function () {
                ...
                if ($scope.inputLabel.labelFilter !== '') {
                    ...
                    for (i = 0; i <= len - 1; i++) {
                        ...
                        if (typeof sim[i][attrs.groupProperty] !== 'undefined' && sim[i][attrs.groupProperty] === false) {
                            $scope.filteredModel.push(sim[i]);
                        }

                        // if it's data
                        var gotData = false;
                        if (typeof sim[i][attrs.groupProperty] === 'undefined') {

                            // If we set the search-key attribute, we use this loop.
                            if (typeof attrs.searchProperty !== 'undefined' && attrs.searchProperty !== '') {

                                //if (re.test(sim[i]['name'])) {
                                //    console.log(sim[i]['name'], keyword, re);
                                //    gotData = true;
                                //    break;
                                //}

                                for (var key in sim[i]) {
                                    if (
                                        typeof sim[i][key] !== 'boolean'
                                        && String(sim[i][key]).toUpperCase().indexOf(keyword) >= 0
            ...
            $scope.getFormElements = function () {
                ...
                if ($scope.helperStatus.all || $scope.helperStatus.none || $scope.helperStatus.reset) {
                    ...
                    if ($scope.helperStatus.filter) {
                        ...
                        inputField = angular.element(element[0]).find('input.inputFilter');
                        clearButton = angular.element(element[0]).find('button.clearButton');
                    }
                }
            ...
            $scope.refreshButton = function () {
                ...
                if ($scope.outputModel.length === 0) {
                    ...
                    if ($scope.more === true) {
                        ...
                        if (tempMaxLabels > 0) {
                            //$scope.varButtonLabel += ', ... '; //PavaniKa 05/07/2015
                            $scope.varButtonLabel = '';
                        }
            ...
            $scope.toggleCheckboxes = function (e) {
                ...
                // open
                    ...
                    var $inputFilter = angular.element(element[0]).find('.inputFilter');

                    // focus on the filter element on open.
                    if ($inputFilter.length > 0) {
                        $inputFilter.focus();
                        //$inputFilter.parent().addClass('is-focus');
    ...
    var template =
        ...
        '<div ng-if="ownerToolTip" tooltip="{{item.fname}}" tooltip-append-to-body="true" tooltip-placement="right">' +
