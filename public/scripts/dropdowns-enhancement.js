...
(function ($) {
    ...
    proto.change = function (e) {
        ...
        if ($items.length) {
            ...
            //This is modified as per TB requirement!
    ...
    ///////////////////////////////////////////
    //START: Traffic Bridge custom functions //
    ///////////////////////////////////////////
    $.fn.dropdown.UpdateSelectionsForDropdown = function(dropdown) {
        var menuClass = 'dropdown-menu';
        var $menu = $(dropdown).find('.' + menuClass);


        //Reset the name/text to 'All'
        var $toggle = $menu.parent().find('[data-label-placement]');
        if (!$toggle || !$toggle.length) {
            $toggle = $menu.parent().find('[data-toggle="dropdown"]');
        }
        ...
        ($toggle.data('placeholder') == undefined && $toggle.data('placeholder', $.trim($toggle.text())));
        var text =  $.data($toggle[0], 'placeholder');
    ...
    $.fn.dropdown.ResetDropdowns = function() {
        var menuClass = 'dropdown-menu';
        var $dropdowns = $('.filter-group').find('.dropdown');
        ...
        $dropdowns.each(function(idx, dropdown){
            ...
            if ($items.length) {
                ...
                $(dropdown).dropdown.UpdateSelectionsForDropdown($(dropdown));
            }
        });
    };

    $.fn.dropdown.UpdateDropdownSelections = function(dropdownType, dropdownValueToUncheck) {
        var menuClass = 'dropdown-menu';
        var $dropdown = $('.filter-group').find('#' + dropdownType + '-DROPDOWN');
        var $menu = $dropdown.find('.' + menuClass);
        var $items = $menu.find('li > input:checkbox');

        if ($items.length) {
            $items.each(function (idx, item) {
                //var item = this;
                if (item.value === dropdownValueToUncheck) {
                    if(item.checked) {
                        item.checked = false;
                    } else {
                        item.checked = true;
    ...
    /////////////////////////////////////////
    //END: Traffic Bridge custom functions //
    /////////////////////////////////////////
